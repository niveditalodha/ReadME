...
def send_email(details):
    ...
    :param details: contains the information about users
    :type: dict
    """
    ...
    if len(details) > 0:
        for detail in details:
            ...
            if send_mail(
                ...
                SentArticle.objects.create(
                    user_id=detail["user_id"],
                    article_id=detail["article_id"],
                    concept_ids=detail["concept_ids"],
                )


def cronjob():
    """Sends articles to the user everyday based on the preferences
    A job that runs daily and sends articles
    """
        ...
        for article in article_objs:
            if preference in list(article.concept_ids.split(",")):
                for sent_article_obj in sent_article_objs:
                    ...
                    article_details[l - 1]["preference"] = Concept.objects.filter(
